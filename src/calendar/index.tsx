...
const Calendar = (props: CalendarProps) => {
  ...
  const updateMonth = useCallback(
    (newMonth?: DateTime) => {
      if (!newMonth || sameMonth(newMonth, currentMonth)) {
        return;
      }
  ...
  const addMonth = useCallback(
    (count: number) => {
      // TODO overflow
      const newMonth = currentMonth?.plus({months: count});
  ...
  const handleDayInteraction = useCallback(
    (date: DateData, interaction?: (date: DateData) => void) => {
      ...
      if (
        allowSelectionOutOfRange ||
        (!(minDate && !isGTE(day, DateTime.fromISO(minDate))) && !(maxDate && !isLTE(day, DateTime.fromISO(maxDate))))
      ) {
  ...
  const renderWeek = (days: DateTime[], id: number) => {
    const week = [];

    days.forEach((day: DateTime, id2: number) => {
      week.push(renderDay(day, id2));
    }, this);

    if (props.showWeekNumbers) {
      week.unshift(renderWeekNumber(days[days.length - 1].weekNumber));
    }
  ...
  const shouldDisplayIndicator = useMemo(() => {
    if (currentMonth) {
      const lastMonthOfDay = toMarkingFormat(currentMonth.endOf('month'));
